...
open class RibCustomisationDirectoryImpl(
    ...
    override fun <T : RibRoutingConfig> put(key: KClass<T>, value: T) {
        map[key] = value
    }
    ...
    override fun <T : RibRoutingConfig> get(key: KClass<T>): T? =
        map[key] as? T

    override fun <T : RibRoutingConfig> getRecursively(key: KClass<T>): T? =
