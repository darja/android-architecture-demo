...
class BackStackFeatureTest {
    ...
    fun `Initial state matches initial configuration`() {
        assertEquals(initialConfiguration, backStack.state.current!!.routing.state)
    }
    ...
    fun `State is updated when operation is applicable`() {
        ...
        val expected = newBackStack.map { it.routing.state }
        val actual = backStack.state.elements.map { it.routing.state }
